...
module.exports = {
    ...
    install: async () => {
        ...
        let removeFileArray = [
           ...
           "/public/json/cluster/clusters.json"
        ]
        ...
        try {
            fs.writeFileSync(updateZipPath, await download(`https://github.com/Kyri123/KAdmin-ArkLIN2/archive/${CONFIG.updater.useBranch}.zip`))
            ...
            fs.createReadStream(updateZipPath)
                ...
                .on("close", () => {
                    ...
                    for(let file of removeFileArray) {
                        let filePath    = pathMod.join(mainDir, "tmp", `KAdmin-ArkLIN2-${branch}`, file)
                    ...
                    fse.copySync(pathMod.join(mainDir, "tmp", `KAdmin-ArkLIN2-${branch}`), mainDir,{ overwrite: true }, (err) => {
                    ...
                    fs.chmodSync(pathMod.join(mainDir, "installer.sh"), 755)
                    fs.chmodSync(pathMod.join(mainDir, "updater.sh"), 755)
                    fs.chmodSync(pathMod.join(mainDir, "starter.sh"), 755)
    ...
    check: () => {
        ...
        // prüfe BranchBuild
        try {
            global.buildIDBranch = Buffer.from(JSON.parse(syncRequest('GET', `https://api.github.com/repos/Kyri123/KAdmin-ArkLIN2/contents/build?ref=${branch}`, {
                headers: {
